...
*/
LIST_ELEM * insertFirst(LIST_HEADER * header, int value, int length){
    if(!header) return NULL;
...
* returns: pointer to new element
*/
LIST_ELEM * insertLast(LIST_HEADER * header, int value, int length){
    if(!header) return NULL;
    LIST_ELEM * new = malloc(sizeof(LIST_ELEM));
    if(!new) return NULL;
...
void printList(LIST_HEADER * header, FILE * fp){
    ...
    for(int i = 0; i < header->len; i++){
        fprintf(fp, "%i: data: Value: %i Length: %i MorseValue: \"%c\"\n", i, cur->value, cur->length, cur->morseValue);
        cur = cur->next;
    }
